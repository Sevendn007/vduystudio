...
function Mark3D({ layers = 60, className, alt = "" }: { layers?: number; className?: string; alt?: string }) {
  const [url, setUrl] = useState<string | null>(null);
  useEffect(() => {
    let mounted = true;
...
.pm-mark3d{position:relative;width:clamp(160px,24vw,340px);perspective:1300px;z-index:2;flex-shrink:0;}
...
.pm-mark-stack img.side{filter:brightness(1.5) saturate(1.2);opacity:0.12;}
...
.pm-word3d{position:relative;display:inline-block;z-index:10;
 font-family:'Anton',sans-serif;font-weight:400;font-size:clamp(60px,16.5vw,260px);line-height:0.85;letter-spacing:.02em;}
.pm-word3d.left {flex:1;text-align:right;margin-right:-5vw;margin-top:-2vw;}
.pm-word3d.right {flex:1;text-align:left;margin-left:-5vw;margin-top:-2vw;}
